...
static const float spHeight = 0.3f;
...
void initFollowWallProjet3()
{
  ...
  // idDown = logGetVarId("deck", "down");

  // Getting the Logging IDs of the state estimates
  idStabilizerYaw = logGetVarId("stabilizer", "yaw");
...
{
  // For our projet 3, we set this variable to skip the hand detection part
...
void endExploration()
{
  // We reuse the existing Velocity and angle parameters to avoid weird behaviour. However,
  // they are divided by 1000.0f to minimize their speed effects in a certain direction
  // setVelocitySetpoint(&setpoint, cmdVelX / 1000.0f, cmdVelY/ 1000.0f, 0.1f, cmdAngWDeg);
  setVelocitySetpoint(&setpoint, 0.0f, 0.0f, 0.1f, cmdAngWDeg);
  commanderSetSetpoint(&setpoint, 3);
}

// Function used for return to base Projet3.
